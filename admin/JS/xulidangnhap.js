...
for (var i = 0; i<localStorage.length; ++i) 
{
	...
	'<div style="clear:both;">'
			...
			+'<div style="float:left;background-color:LightGray;width:30%;border-right:solid 1px; padding-bottom:3px">'
			+localStorage.key(i)+'</div>'
			+'<div style="float:left;background-color:LightGray; width:30%;padding-bottom:3px;border-right:solid 1px;">'+localStorage.getItem ( localStorage.key(i) )+'</div>'
			+'<div style="float:left;background-color:LightGray; width:10%;border-right:solid 1px">'+'<button onclick="return xoalocal('+i+')">Xóa</button>'+'</div>'
			+'<div style="float:left;background-color:LightGray;width:29.6% ">'+'<button onclick="return tdmklocal('+i+')">Reset mật khẩu về 123456</button>'+'</div>'
			+'</div>'
	;
	}
	else{
		
		quanli+='<div style="clear:both;">'
			
			+'<div style="float:left;width:30%;border-right:solid 1px;padding-bottom:3px; border-right:solid 1px ">'
			+localStorage.key(i)+'</div>'
			+'<div style="float:left; width:30%; padding-bottom:3px; border-right:solid 1px">'+localStorage.getItem ( localStorage.key(i) )+'</div>'
			+'<div style="float:left; background-color:white;color:black;width:10%;border-right:solid 1px">'+'<button onclick="return xoalocal('+i+')">Xóa</button>'+'</div>'
			+'<div style="float:left; background-color:white;color:black;width:29.6%;">'+'<button onclick="return tdmklocal('+i+')">Reset mật khẩu về 123456</button>'+'</div>'
			+'</div>'
	;
		}
